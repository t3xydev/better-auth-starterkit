...
| Invite-only flag | `NEXT_PUBLIC_INVITE_ONLY` via [`src/lib/invite-only.ts`](../src/lib/invite-only.ts) |
...
1. Promote an admin (see [admin panel](./admin-panel.md)). With invite-only enabled, create the first account before (or as) the bootstrap user, then promote.
