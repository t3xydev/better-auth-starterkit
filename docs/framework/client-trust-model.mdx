...
description: "Trust tiers, DCR defaults, and scope gating."
---

> Part of the [docs index](/docs/framework). Related: [admin panel](/docs/framework/admin-panel) · [features](/docs/framework/features) · [environment variables](/docs/framework/environment-variables).
...
DCR clients are registered against the default protected resource (`OAUTH_AUDIENCE` or `BETTER_AUTH_URL`) via `clientRegistrationDefaultResources` / `clientRegistrationAllowedResources`. Per-client resource enforcement stays on — a token request for an unlinked resource is rejected with `invalid_target`.
