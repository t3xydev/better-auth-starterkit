---
title: "Admin panel"
description: "Create and manage OAuth / OIDC clients."
---

> Part of the [docs index](/docs/framework). Related: [features](/docs/framework/features) · [environment variables](/docs/framework/environment-variables).
...
Also available: **Invites** at `/admin/invites` — see [invitations.md](/docs/framework/invitations).
...
2. Fill in the required fields:
   ...
   - **Post-logout redirect URIs** -- optional exact callback allowlist for RP-initiated logout, one per line.
3. Choose a **trust tier** (defaults to Registered developer). See [client-trust-model.md](/docs/framework/client-trust-model).
4. Optionally toggle:
...
| **OAuth Configuration** | Redirect URIs, Post-logout Redirect URIs, Scopes (gated by trust tier), Grant Types (`client_credentials` requires partner+) |
...
When **Enable End Session** is active, add every exact value an application may send as `post_logout_redirect_uri`. The server redirects only to allowlisted values; web HTTPS URLs and native custom-scheme callbacks are supported. This setting is stored per client as `post_logout_redirect_uris`, so future clients can be configured entirely from the panel without a data migration.
