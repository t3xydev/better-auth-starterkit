...
`pnpm dev` and `pnpm start` apply committed migrations on boot. After a fresh clone, start the app once Postgres is reachable — the admin columns (`role`, `banned`, …) are already in those migrations.
