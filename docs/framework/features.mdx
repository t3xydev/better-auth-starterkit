...
|---|---|---|
| Email code | `emailOTP` | Preferred passwordless sign-in at `/auth/sign-in`; password remains available at `/auth/password`. Codes expire after five minutes, are stored hashed, and cannot create unknown accounts |
...
| Passkeys | `@better-auth/passkey` | WebAuthn; RP ID derived from `BETTER_AUTH_URL`; an enrolled passkey can complete a pending 2FA challenge for the same account |
| Two-factor (TOTP) | `twoFactor` | Challenge UI at `/auth/two-factor`; a valid code repairs legacy enabled accounts with an unverified TOTP record |
| Nostr | `better-auth-nostr` + custom link plugin | Sign-in and account key linking (`/account/nostr`) |
| Invitations | `better-invite` | Admin create/list UI, email + public invites, `/invite/activate/[token]` welcome + accept flow — [guide](/docs/framework/invitations). Opt-in invite-only registration via `NEXT_PUBLIC_INVITE_ONLY=true` |
...
Same account model as above — first-class sign-in / link paths for the IdP (see [product.md](/docs/framework/product)):
...
| Admin UI for clients | `/admin/clients` — [guide](/docs/framework/admin-panel) |
...
Dynamically registered clients start as **Tier 0 (unknown)**: public scopes only, mandatory consent, PKCE, no machine grants, and an “unverified” warning on the consent screen. See [client-trust-model.md](/docs/framework/client-trust-model).
...
JWT issuer defaults to `BETTER_AUTH_URL` (overridable with `OAUTH_ISSUER`). Access tokens are bound to one protected resource — `OAUTH_AUDIENCE` when set, otherwise `BETTER_AUTH_URL`. That identifier is the JWT `aud` claim.

OAuth protocol errors redirect to `/auth/error`, which shows `error` / `error_description` and a link back to sign-in.

Authorization-code **ID tokens no longer include profile or email claims**. Client apps should read those from UserInfo (`/api/auth/oauth2/userinfo`).
...
- In-app docs via Fumadocs at `/docs/framework` (opt-in with `NEXT_PUBLIC_DOCS_ENABLED=true`)
...
4. Deploy. `pnpm start` (and container boot) applies pending migrations.

See [deployment.md](/docs/framework/deployment) and [getting-started.md](/docs/framework/getting-started).
