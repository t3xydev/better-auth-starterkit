...
description: "Secrets, SMTP, branding, and analytics."
---

> Part of the [docs index](/docs/framework). Copy `.env.example` for a commented template.
...
| `OAUTH_AUDIENCE` | Protected resource identifier and JWT `aud` for access tokens. Also used as the default (and only allowed) resource for Dynamic Client Registration. Falls back to `BETTER_AUTH_URL`. | `BETTER_AUTH_URL` |
...
| Variable | Description | Default |
|----------|-------------|---------|
| `NEXT_PUBLIC_SHOW_GITHUB` | Set to `"true"` to show the GitHub link in the header. | `false` (hidden) |
| `NEXT_PUBLIC_DOCS_ENABLED` | Set to `"true"` to enable the Fumadocs site at `/docs/framework` (header link, routes, search). | `false` |
...
| `NEXT_PUBLIC_INVITE_ONLY` | Set to `"true"` to require a [better-invite](/docs/framework/invitations) link for new registrations. The first user may still sign up to bootstrap an admin. | `false` |
