...
> Part of the [docs index](/docs). Copy `.env.example` for a commented template.
...
| `NEXT_PUBLIC_DOCS_ENABLED` | Set to `"true"` to enable the Fumadocs site at `/docs` (header link, routes, search). | `false` |
...
| `NEXT_PUBLIC_INVITE_ONLY` | Set to `"true"` to require a [better-invite](/docs/invitations) link for new registrations. The first user may still sign up to bootstrap an admin. | `false` |
