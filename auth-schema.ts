...
export const dbscSessions = pgTable("dbsc_sessions", {
  ...
  lastRefreshAt: integer("last_refresh_at").notNull(),
});

export const dbscBoundKeys = pgTable("dbsc_bound_keys", {
  id: text("id").primaryKey(),
  sessionId: text("session_id")
    .notNull()
    .references(() => dbscSessions.id, { onDelete: "cascade" }),
...
export const devtoolsUsers = pgTable("devtools_users", {
  id: text("id").primaryKey(),
  userId: text("user_id")
    .notNull()
    .unique()
...
export const usersRelations = relations(users, ({ one, many }) => ({
  ...
  dbscSessions: many(dbscSessions),
  devtoolsUsers: one(devtoolsUsers),
}));
...
export const oauthClientResourcesRelations = relations(
  ...
  ({ one }) => ({
    ...
    oauthResources: one(oauthResources, {
...
export const dbscSessionsRelations = relations(dbscSessions, ({ one, many }) => ({
  users: one(users, {
    fields: [dbscSessions.userId],
    references: [users.id],
  }),
  dbscBoundKeys: many(dbscBoundKeys),
}));

export const dbscBoundKeysRelations = relations(dbscBoundKeys, ({ one }) => ({
  dbscSessions: one(dbscSessions, {
    fields: [dbscBoundKeys.sessionId],
    references: [dbscSessions.id],
  }),
}));

export const devtoolsUsersRelations = relations(devtoolsUsers, ({ one }) => ({
  users: one(users, {
    fields: [devtoolsUsers.userId],
    references: [users.id],
  }),
}));
